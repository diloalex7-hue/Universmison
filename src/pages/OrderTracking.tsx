...
export default function OrderTracking() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (!data) setErr("Commande introuvable. Vérifiez votre numéro.");
    else setOrder(data);
  };
  ...
  return (
    <div className="container-luxe py-12">
      <div className="mx-auto max-w-xl text-center">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gradient-primary"><Package className="h-5 w-5 text-gold" /></div>
        <h1 className="mt-4 font-serif text-4xl">Suivi de commande</h1>
        <p className="mt-2 text-sm text-muted-foreground">Saisissez votre numéro de commande (ex: UM-20260502-abc123)</p>
      ...
      {order && (
        <div className="mx-auto mt-10 max-w-2xl rounded-2xl border border-border bg-card p-8 shadow-elegant">
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <div key={s} className="flex flex-col items-center text-center">
                    <div className={`flex h-9 w-9 items-center justify-center rounded-full text-xs font-bold ${done ? "bg-gradient-gold text-gold-foreground" : "bg-secondary text-muted-foreground"}`}>{i + 1}</div>
                    <div className="mt-2 text-[11px] capitalize text-muted-foreground">{s}</div>
                  </div>
                );
              })}
            </div>
